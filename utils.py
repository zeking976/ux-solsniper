...
def get_sol_price_usd(timeout: int = 6, force_refresh: bool = False) -> float:
    ...
    try:
        ...
        r = requests.get(cg_url, params=params, timeout=timeout)
        r.raise_for_status()
        data = r.json()
        if isinstance(data, dict) and "solana" in data and "usd" in data["solana"]:
    ...
    if not price:
        try:
            ...
            elif isinstance(data, dict) and "SOL" in data:
    ...
    if price <= 0:
        logger.warning("get_sol_price_usd: price unavailable, returning 0.0")
        return 0.0
    return float(price)
